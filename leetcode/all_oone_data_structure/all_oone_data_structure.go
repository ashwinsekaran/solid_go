...
//   Inc: move key to next bucket (count+1), create bucket if needed
//   Dec: move key to prev bucket (count-1), delete if count reaches 0
//   GetMaxKey: peek at tail bucket (highest count)
//   GetMinKey: peek at head bucket (lowest count)
...
func main() {
	...
	obj.Inc("c")
	obj.Inc("c")
	fmt.Println("MaxKey:", obj.GetMaxKey()) // c
	fmt.Println("MinKey:", obj.GetMinKey()) // a
	obj.Dec("c")
	obj.Dec("c")
	obj.Dec("c")
	fmt.Println("MaxKey after dec c:", obj.GetMaxKey()) // b
	fmt.Println("MinKey after dec c:", obj.GetMinKey()) // a
}
